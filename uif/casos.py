...
def obtener_caso(caso_id):
    with get_db() as db:
        query = "SELECT * FROM casos WHERE caso_id = :caso_id"
        return db.execute(query, {'caso_id': caso_id}).fetchone()
...
def obtener_personas_caso(caso_id):
    with get_db() as db:
        ...
        """
        return db.execute(query, {'caso_id': caso_id}).fetchall()
...
def eliminar_persona_de_caso(caso_id, persona_id):
    with get_db() as db:
        query = "DELETE FROM casos_personas WHERE caso_id = :caso_id AND persona_id = :persona_id"
