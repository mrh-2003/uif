    ...
    with tab2:
        ...
        if casos:
            ...
            if caso_seleccionado:
                ...
                st.write(f"**Estado:** {caso.get('estado')}")
                st.write(f"**Prioridad:** {caso.get('prioridad')}")
                st.write(f"**Descripción:** {caso.get('descripcion')}")
...
def pagina_analisis_transaccional():
    ...
    st.info(f"Caso Actual: {caso.get('nombre_caso')}")
